...
mod tests {
    ...
    async fn test_oem_manager_profiles_passed_to_machine_setup() {
        ...
        type_profiles.insert(BiosProfileType::Performance, attrs);
        let mut model_profiles = HashMap::new();
        model_profiles.insert("r760".to_string(), type_profiles);
        let mut oem_profiles = HashMap::new();
        oem_profiles.insert(RedfishVendor::Dell, model_profiles);

        let mut config = crate::tests::common::api_fixtures::get_config();
        config.oem_manager_profiles = oem_profiles.clone();
        ...
        assert_eq!(
            ...
            RedfishSimAction::MachineSetup {
                oem_manager_profiles: oem_profiles,
            }
        );
    }
